...
class AuthConnectorForITests @Inject()(http: WSHttp, appConfig: AppConfig) extends Logging {
  ...
  def loginAndGetToken(authUserDetails: AuthUserDetails)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Either[String, String]] = {
    http.post(appConfig.authLoginStubUrlForITests, getRequestBody(authUserDetails)).map {
      response ⇒
  ...
  private def getGrantScopePage(response: HttpResponse)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Either[String, String]] = {
    ...
    http.get(s"http://oauth-frontend.public.mdtp:80$oauthGrantScopeUrl", Map("Cookie" -> getMdtpCookie(response))).map {
      response ⇒
        response.status match {
          case Status.OK =>
  ...
  private def postGrantScope(response: HttpResponse)(implicit hc: HeaderCarrier, ec: ExecutionContext) = {
    ...
    http.doFormPost("http://oauth-frontend.public.mdtp:80/oauth/grantscope", Map("auth_id" -> Seq(authId)))(hc.withExtraHeaders(headers.toSeq: _*)).map {
      response =>
        response.status match {
          case Status.OK =>
